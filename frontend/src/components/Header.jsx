...
export default function Header({ showBackButton = false, onLogout, user, right }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-br from-[#6DD5ED] to-[#2196F3] bg-opacity-95 backdrop-blur-lg border-b border-transparent shadow-sm text-white">
      ...
      <div className="absolute left-3 sm:left-4 md:left-6 top-1/2 transform -translate-y-1/2 flex items-center">
      ...
      <div className="absolute right-3 sm:right-4 md:right-6 top-1/2 transform -translate-y-1/2 flex items-center space-x-2 sm:space-x-3">
        {showBackButton && (
          <button
            onClick={() => navigate('/')}
            className="inline-flex items-center px-2.5 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium text-white bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg transition-all duration-200 hover:shadow-md active:scale-95"
          >
            <svg
        ...
        {onLogout && (
          <button
            onClick={onLogout}
            className="inline-flex items-center px-2.5 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium text-white bg-[#DC3545] hover:bg-[#c82333] rounded-lg transition-all duration-200 hover:shadow-lg active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#f5c6cb]"
          >
            <svg
              className="w-3.5 h-3.5 sm:w-4 sm:h-4 sm:mr-2"
              fill="none"
